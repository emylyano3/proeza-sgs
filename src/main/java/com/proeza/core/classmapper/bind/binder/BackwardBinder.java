...
@Component
public class BackwardBinder implements Binder {
	...
	public <M extends Mapeable> void bind (M mapeable, Object binded, BindReader bindReader) {
		try {
			...
			if (bindReader.isBackwardBindDefined()) {
				Object value = util.getFieldValue(bindedField.getField(), bindedField.getObject());
				util.invokeBackwardMethod(mapeable, bindReader.getBackwardBinder(), value);
				return;
			}
